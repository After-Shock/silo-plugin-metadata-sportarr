...
grep -Fq 'match-candidates.json' "$smoke" || fail 'smoke does not retain match candidates for assertion diagnosis'
grep -Fq 'match-apply.json' "$smoke" || fail 'smoke does not retain match-apply response for assertion diagnosis'
grep -Fq 'persisted-item.json' "$smoke" || fail 'smoke does not retain the persisted Movie item for assertion diagnosis'
grep -Fq 'silo.log' "$smoke" || fail 'smoke does not retain Silo logs for assertion diagnosis'
grep -Fq 'assert_persisted_provider_id()' "$smoke" || fail 'smoke does not verify the durable provider-id record'
grep -Fq 'media_item_provider_ids' "$smoke" || fail 'smoke does not query the durable provider-id table'
grep -Fq "content_id = :'content_id'" "$smoke" || fail 'provider-id query does not safely bind the content ID'
grep -Fq "provider_id = :'provider_id'" "$smoke" || fail 'provider-id query does not safely bind the provider ID'
